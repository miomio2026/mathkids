...
// Calcola il risultato rispettando la precedenza: × e ÷ prima di + e −
function evalWithPrecedence(nums, ops) {
  let n = [...nums];
  let o = [...ops];
  // Prima passata: × e ÷
  let i = 0;
  while (i < o.length) {
    if (o[i] === 'mul' || o[i] === 'div') {
      const res = o[i] === 'mul' ? n[i] * n[i + 1] : n[i] / n[i + 1];
      n.splice(i, 2, res);
      o.splice(i, 1);
    } else {
      i++;
  ...
  for (let j = 0; j < o.length; j++) {
    total = o[j] === 'add' ? total + n[j + 1] : total - n[j + 1];
  }
  return total;
}
